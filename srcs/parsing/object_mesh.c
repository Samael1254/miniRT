...
static void transform_mesh(t_mesh *mesh, t_vec3 pos, t_vec3 rot, double scale) {
  ...
  while (i < mesh->n_vertices) {
    mesh->vertices[i] =
        ft_4to3_vec(ft_rotate_vec4(ft_3to4_vec(mesh->vertices[i]), rot));
  ...
  while (i < mesh->n_normals) {
    mesh->normals[i] =
        ft_4to3_vec(ft_rotate_vec4(ft_3to4_vec(mesh->normals[i]), rot));
    i++;
  }
}

void mesh_params(t_state *state, t_object *obj, char **split, bool *has_error) {
  t_vec3 pos;
  t_vec3 rot;
...
t_object *object_mesh(t_state *state, char **split) {
  ...
  if (!obj)
    return (ft_free_strtab(split),
            error("malloc failed", "in object_mesh", state), NULL);
