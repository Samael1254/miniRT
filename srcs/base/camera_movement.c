...
void rotate_camera(t_state *state, t_camera *camera, enum e_keycode keycode) {
  ...
  camera->dir =
      ft_4to3_vec(ft_mat_vec_product4(m_rot, ft_3to4_vec(camera->dir)));
  if (keycode == A_KEY || keycode == D_KEY)
    camera->x_axis =
        ft_4to3_vec(ft_mat_vec_product4(m_rot, ft_3to4_vec(camera->x_axis)));
  camera->y_axis =
      ft_4to3_vec(ft_mat_vec_product4(m_rot, ft_3to4_vec(camera->y_axis)));
