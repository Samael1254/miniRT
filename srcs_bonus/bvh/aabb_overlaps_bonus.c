...
#include "minirt_aabb_overlap_bonus.h"
...
static bool	aabb_aabb_overlap(t_aabb a, t_aabb b)
{
...
static bool	plane_aabb_overlap(t_vec3 p_normal, t_vec3 p_point, t_aabb box)
{
	t_vec3	c;
	t_vec3	h;
	double	r;
...
static bool	test3(t_vec3 r_vertices[3], t_vec3 edges[3], t_vec3 extent,
		t_vec3 unit[3])
{
...
static t_aabb	create_triangle_aabb(const t_vec3 vertices[3])
{
	unsigned int	i;
	t_aabb			aabb;
		...
		const t_vec3 *vertices)
{
	...
	r_vertices[0] = ft_sub_vec3(vertices[vertices_id[0]], center);
	r_vertices[1] = ft_sub_vec3(vertices[vertices_id[1]], center);
	r_vertices[2] = ft_sub_vec3(vertices[vertices_id[2]], center);
	...
	if (!test3(r_vertices, edges, extent, unit))
		return (false);
	if (!aabb_aabb_overlap(box, create_triangle_aabb(r_vertices)))
		return (false);
	if (!plane_aabb_overlap(ft_cross_vec3(edges[0], edges[1]), r_vertices[0],
			box))
		return (false);
	return (true);
}
